...
// PackSuspendApp is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x8160c73e.  This method will panic if any
// invalid/nil inputs are passed.
//
// Solidity: function suspendApp(address app) returns()
func (appController *AppController) PackSuspendApp(app common.Address) []byte {
	enc, err := appController.abi.Pack("suspendApp", app)
	if err != nil {
		panic(err)
	}
	return enc
...
// TryPackSuspendApp is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x8160c73e.  This method will return an error
// if any inputs are invalid/nil.
//
// Solidity: function suspendApp(address app) returns()
func (appController *AppController) TryPackSuspendApp(app common.Address) ([]byte, error) {
	return appController.abi.Pack("suspendApp", app)
}

// PackSuspendAppByAdmin is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x762f58e5.  This method will panic if any
// invalid/nil inputs are passed.
//
// Solidity: function suspendAppByAdmin(address app) returns()
func (appController *AppController) PackSuspendAppByAdmin(app common.Address) []byte {
	enc, err := appController.abi.Pack("suspendAppByAdmin", app)
	if err != nil {
		panic(err)
	}
	return enc
...
// TryPackSuspendAppByAdmin is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x762f58e5.  This method will return an error
// if any inputs are invalid/nil.
//
// Solidity: function suspendAppByAdmin(address app) returns()
func (appController *AppController) TryPackSuspendAppByAdmin(app common.Address) ([]byte, error) {
	return appController.abi.Pack("suspendAppByAdmin", app)
}
...
const AppControllerAppSuspendedByAdminEventName = "AppSuspendedByAdmin"

// ContractEventName returns the user-defined event name.
func (AppControllerAppSuspendedByAdmin) ContractEventName() string {
	return AppControllerAppSuspendedByAdminEventName
}

// UnpackAppSuspendedByAdminEvent is the Go binding that unpacks the event data emitted
// by contract.
//
// Solidity: event AppSuspendedByAdmin(address indexed app)
func (appController *AppController) UnpackAppSuspendedByAdminEvent(log *types.Log) (*AppControllerAppSuspendedByAdmin, error) {
	event := "AppSuspendedByAdmin"
	if len(log.Topics) == 0 || log.Topics[0] != appController.abi.Events[event].ID {
		return nil, errors.New("event signature mismatch")
	}
	out := new(AppControllerAppSuspendedByAdmin)
	if len(log.Data) > 0 {
		if err := appController.abi.UnpackIntoInterface(out, event, log.Data); err != nil {
			return nil, err
		}
